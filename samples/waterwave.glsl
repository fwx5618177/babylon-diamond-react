...
vec3 rotateX(vec3 p,float angle){
    return(vec4(p,1.)*rotationAxis(angle,vec3(1.,0.,0.))).xyz;
}

vec3 rotateY(vec3 p,float angle){
    return(vec4(p,1.)*rotationAxis(angle,vec3(0.,1.,0.))).xyz;
}

vec3 rotateZ(vec3 p,float angle){
    return(vec4(p,1.)*rotationAxis(angle,vec3(0.,0.,1.))).xyz;
}
...
void main(void){
    ...
    v_position.xyz=rotateZ(
        v_position.xyz,
        cos(u_time+a_position.x)*sin(u_time*4.+a_position.y)*.2
    );
    #endif
...
vec2 tile(in vec2 p,vec2 w){return fract(mod(p+w/2.,w))-(w/2.);}
vec2 tile(in vec2 p,float w){return tile(p,vec2(w));}

float grid(vec2 p){
    vec2 grid=abs(fract(p-.5)-.5)/fwidth(p);
...
float noise(in vec2 p,in float scale){
    ...
    p=floor(p);
    f=f*f*(3.-2.*f);// Cosine interpolation approximation
    float res=mix(mix(hash(p,scale),
    hash(p+vec2(1.,0.),scale),f.x),
    mix(hash(p+vec2(0.,1.),scale),
    hash(p+vec2(1.,1.),scale),f.x),f.y);
    return res;
}
float fbm(in vec2 p){
...
void main(){
    ...
    vec2 dxy=dHdxy(t,.02);
    normal=bumpNormal(vec3(t,1.),normal,dxy);
    #endif
    ...
    float incidence=max(dot(normal,direction),-1.);
    vec3 light=clamp(ambient+lightColor*incidence,0.,1.);
    
    // fresnel
    float fresnel=1.-dot(normal,vec3(0.,0.,1.));
    ...
    float hatch=clamp(fresnel-incidence,0.,1.);
    hatch=smoothstep(hatch,0.,.2);
    #if defined(useHatch)
    hatch*=step(length(tile(st*70.,.9))*2.,.5);
    #endif
    #else
    float hatch=0.;
    #endif
    ...
    // grid
    #if defined(useGrid)
    float gridColor=max(grid(p*4.)*.6,grid(p*8.)*.3);
    light*=(1.-gridColor);
    #endif
    
    gl_FragColor=vec4(clamp(max(light,color),0.,1.)-hatch,alpha);
